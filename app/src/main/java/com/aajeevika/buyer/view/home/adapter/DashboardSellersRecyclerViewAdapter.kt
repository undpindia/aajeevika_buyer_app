...
import com.aajeevika.buyer.databinding.ListItemProductHorizontalCardBinding
...
import com.aajeevika.buyer.utility.UtilityActions.pxRespectToDeviceWidth
import com.aajeevika.buyer.view.product.ActivityProductDetail

class DashboardSellersRecyclerViewAdapter : BaseRecyclerViewAdapter() {
    ...
    override fun createViewHolder(inflater: LayoutInflater, parent: ViewGroup, viewType: Int): BaseViewHolder = run {
        PopularSellersViewHolder(ListItemProductHorizontalCardBinding.inflate(inflater, parent, false))
    }

    override fun getItemCount() = basicDataList.size

    private inner class PopularSellersViewHolder(private val viewDataBinding: ListItemProductHorizontalCardBinding) : BaseViewHolder(viewDataBinding) {
        override fun bindData(context: Context) {
            val parent = viewDataBinding.root
            val layoutParams = parent.layoutParams
            layoutParams.width = pxRespectToDeviceWidth(parent.context, 1.5F).toInt()
